...
export class BluetoothHelper {
  ...
  public requestDevice(
    ...
    const requestStream = from(
      ...
      tap((device) =>
        this.logger.info(`Device Selected: ${device.name}`, device)
      ),
      catchError((err) => {
    ...
    return new Observable<BluetoothRemoteGATTServer>((observer) => {
      ...
      return {
        unsubscribe: () => {
          ...
          if (server.connected) {
            this.logger.info(
              `connectServer.unsubscribe: Disconnecting from server...`,
              server
            );
            server.disconnect();
          } else {
            this.logger.info(
              `connectServer.unsubscribe: Server already disconnected`,
              server
            );
          }
        },
  ...
  public getService(
    ...
    return from(server.getPrimaryService(service)).pipe(
      ...
      catchError((err) => {
        this.logger.error(
          `Error getting service '${service}' (${retries})`,
          err
        );
  ...
  public createDeviceDisconnectionStream(
    device: BluetoothDevice
  ): Observable<BluetoothDevice> {
